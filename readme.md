...
mixed-criticality present in real-time systems. Implementation extends OpenStack. First, it applies
renewables-driven cores in server CPUs using Intel Power Optimization library. Then it conducts server level load
matching against the availability of renewables, using criticality-aware VM evictions. At the control plane,
a custom VM scheduling algorithm reduces the possibilities of evictions incidents, while maximizing the
...
At least two machines, one will run the control plane, and other runs the compute.

For compute nodes,

- Linux-based OS
- Intel CPU with the support for sleep states and dynamic frequency scaling features
- At least 2 CPU cores
- CPU hyper-threading disabled

Deployment should be done in a clean environment to avoid deployment issues.
Compute nodes should be bare-metal as power controller module needs to control per-core power profile.

### Deployment guide

First decide an IP plan. We use
...
Following configs are common for both Control node and compute nodes. So do that in all nodes first.
...
5. Devstack downloads nova code and then use that during the stack.sh script runtime. We need to modify its code before.
   that. So we first do a dry run (./stack.sh -> unstack.sh). This will have files downloaded to local. We then apply
   the
...
Note: If default location of openstack is going to be changed, following configs needs to be added to the `local.conf`
of
...
4. To enforce Green core packing algorithm, open `/etc/nova/nova.conf` and make sure `weight_classes` is set just to
   CPUWeighter.
   ```bash
   [filter_scheduler]
   ...
...
2. Now, we have a working Openstack-GC deployment. Let's create a VM. First, lets create a flavor for pinned cores. Log
   into dashboard and select the
...
3. In the dashboard, upgrade quota limits. Calculate total number of cores, and set the limit to that number. (admin->
   system->default). Also, the vm flavor should use cpu pinning. So update its metadata of cpu pinning to `dedicated`.
4. Now, create a VM with the flavor `m1.nano`. This will be pinned to the stable
   cores. [create-vm.sh](vm-trace%2Faz-trace-gen%2Fsrc%2Fmain%2Fresources%2Fos-client%2Fcreate-vm.sh)
   will do this, but make sure to download `admin-openrc.sh` from dashboard under `API Access`, and source it in the
   terminal. Since emulation service uses this file to
   authenticate with Openstack APIs, stop and restart the emulation service once the file is sourced. Also, find the
   public network's ID from dashboard and update the `create-vm.sh` script.
...
7. Now switch the renewables-driven cores to sleep through emulation API. This should force delete all VMs occupied renewables-driven cores.
   ...
   ```
8. Ta da! You have a working Openstack-GC deployment. Try to launch an instance with the demo project, create and assign
...
Following tables outline the experimental setup. We use a HPE ProLiant server with following configurations.
...
For the workload, we deploy VMs with following configurations, and test for the real-time performance of the VM kernel.
...
of renewable energy. During the experiment, we measure for CPU metrics using Intel's Running Average Power Limit (RAPL)
...
effectiveness. Experiments at scale is a part of our next release.
...
cores such as the 0th one. These needs to be tested first). Also, in load matching at-scale, VM evictions by the 
control plane benefits from VM criticality. These works are part of the next release.

Currently, renewable capacity change between two states, such that renewables-driven cores are either all
asleep or all awake. A more granular state changes will be available in the next release.
